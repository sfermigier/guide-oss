...
Compiere est distribué sous la licence MPL et est écrit sur une base Java.
...
ERP5 est distribué sous licence GPL.

ERP5 est écrit en Python et est basé sur la base objets Zope, ce qui malgré les qualités de cette dernière n’est pas du niveau des meilleures bases de données relationnelles.
...
OFBiz est distribué sous la licence Apache.
...
Openbravo est publié sous la licence publique Openbravo basée sur la licence open source MPL (1.1). L’éditeur est solide et apte à fournir un support professionnel.
...
OpenERP est distribué sous licence AGPL 3.0 (depuis la version 6).

OpenERP est écrit en Python et repose sur un framework orienté objet puissant. La base de données PostgreSQL est utilisée.
